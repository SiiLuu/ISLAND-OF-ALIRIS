...
void check_gardin_interaction(global_t *global)
{
    if (sfKeyboard_isKeyPressed(sfKeyA) &&
        ...
        global->gameplay->pnj->status->p_garden = 1;
        }
    if (global->gameplay->pnj->status->p_garden == 1 &&
        ((global->gameplay->x < 2450 || global->gameplay->x > 2600) ||
        (global->gameplay->y < 900 || global->gameplay->y > 1020))) {
        global->gameplay->pnj->status->p_garden = 0;
...
void check_interaction_village(global_t *global)
{
    if (sfKeyboard_isKeyPressed(sfKeyA) &&
        ...
        global->gameplay->pnj->status->vil_papy = 1;
        }
    if (global->gameplay->pnj->status->vil_papy == 1 &&
        ((global->gameplay->x < 500 || global->gameplay->x > 710) ||
        (global->gameplay->y < 590 || global->gameplay->y > 750))) {
        global->gameplay->pnj->status->vil_papy = 0;
        global->gameplay->pnj->status->next_text = 0;
...
void display_text_vill(global_t *global)
{
    if (global->gameplay->pnj->status->vil_papy == 1) {
        ...
        if (sfKeyboard_isKeyPressed(sfKeyTab))
            global->gameplay->pnj->status->next_text = 1;
        if (global->gameplay->pnj->status->next_text == 1)
